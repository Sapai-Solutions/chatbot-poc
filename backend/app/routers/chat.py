...
async def chat_stream(
    ...
    NOTE: Does NOT use Depends(get_db) because FastAPI closes dependency-
    injected sessions as soon as the endpoint returns the StreamingResponse,
    which is *before* the async generator finishes.  Instead the generator
    manages its own DB session so it stays open for the entire stream.
